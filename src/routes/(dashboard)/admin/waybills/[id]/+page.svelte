<script>
  ...
  function statusStyle(val) {
    switch (val) {
      ...
      case "canceled":
        return "text-error";
      default:
        return "text-text-base-content/80";
    }
  }
  ...
  async function cancelDelivery() {
    const yes = confirm(
      "Do you really want to continue? This action can not be reversed. You can always reassign the delivery."
    );
    if (!yes) return;
  ...
  async function endDelivery() {
    const yes = confirm(
      "Do you really want to continue? This action can not be reversed."
    );

    if (!yes) return;
    const reason = prompt("Enter reason why you want cancel this delivery");
    showModal();
    isLoading = true;
    try {
      const res = await clientFetch({
        path: `/deliveries/${$item.delivery.id}/return`,
        method: "POST",
        body: {
          reason,
        },
...
<div class="page print:hidden">
  <div class="flex items-center justify-between mb-4">
    <div class="flex flex-1 justify-end gap-3 items-center">
      <div>
        ...
        <Dropdown triggeredBy="#history" placement="bottom" class="w-72 z-[10]">
          <div
            slot="header"
            class="text-center py-3 font-medium bg-slate-50 text-sm"
          >
            Delivery History
          </div>
          <div class="divide-y relative">
            {#each $item.history as dl, index}
              <DropdownItem class="flex gap-3 p-3 items-center">
                <div
                  class="w-6 h-6 rounded-full grid place-content-center bg-slate-100 text-sm"
                >
                  {index + 1}
                </div>
                <div class="flex-1 text-sm">
                  ...
                  <div class="flex justify-between">
                    <span class="{statusStyle(dl.status)} font-semibold text-xs"
                      >{computeStatus(dl.status)}</span
                    >
                    <a
                      href="/admin/deliveries/{dl.id}"
                      class="text-xs font-medium hover:underline text-primary"
                      >See more</a
                    >
                  </div>
                </div>
      ...
      <Button
        ...
        disabled={isLoading}
      >
        <span class="loading loading-spinner w-3" class:hidden={!isLoading}
        ></span>
        <span>Update</span>
      </Button>
    </div>
  ...
  {#if $item.delivery && ["en-route", "pending"].includes($item.delivery.status)}
    <Alert color="red">
      <div class="pb-2 text-sm">
        Would you like to return the ongoing delivery?
      </div>
      <Button color="red" on:click={cancelDelivery}>
        {#if isLoading}
    ...
    <Alert color="yellow" class="mt-6">
      <div class="pb-2 text-sm">
        Would you like to cancel the ongoing delivery?
      </div>
      <Button color="red" on:click={endDelivery}>
        {#if isLoading}
          <Spinner size={4} class="mr-3" />
        {/if}
        Cancel Delivery
  ...
  <div class="bg-base-100 rounded-sm shadow-sm">
    <div class="p-8" id="pdf">
      <div
        class="w-full flex flex-col md:flex-row-reverse justify-between gap-8"
      >
          ...
          <div class="grid grid-cols-2 gap-4 py-6">
            <div class="bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <div class="text-xs text-base-content/80">
                {dayjs($item.created_at).format("DD-MM-YYYY [at] HH:mm A")}
              </div>
            </div>
            {#if $item.creator}
              <div class="bg-slate-50 p-4 rounded-xl border border-slate-100">
      ...
      <div
        class="flex-none grid place-content-center bg-accent/5 rounded-full w-11 h-11 mt-8"
      >
        <UilParcel size="20" />
      </div>
      <div class="flex justify-between items-center">
        ...
        <Button
          size="xs"
          color="alternative"
          class="p-2 rounded-full bg-primary/5 text-primary space-x-2"
          on:click={() => (hidePackageInfo = false)}
        >
      ...
      <div class="grid gap-8 md:grid-cols-2">
        <div class="py-5">
          ...
          <div class="info-item">
            ...
            <div class="content">
              <span class:fade={!$item.sender.name}
                >{$item.sender.name || "Not set"}</span
              >
            </div>
          </div>
          <div class="info-item">
            ...
            <div class="content">
              <span class:fade={!$item.sender.email}
                >{$item.sender.email || "Not set"}</span
              >
            </div>
          </div>
          ...
          <div class="info-item">
            ...
            <div class="content">
              <span class:fade={!$item.sender.phone}
                >{$item.sender.phone || "Not set"}</span
              >
            </div>
          </div>
          <div class="info-item">
            ...
            <div class="content">
              <span class:fade={!$item.sender.address}
                >{$item.sender.address || "Not set"}</span
              >
            </div>
          </div>
        </div>
        <div class="py-5">
          ...
          <div class="info-item">
            <span class="title">Name</span>
            <div class="content">
              <span class:fade={!$item.recipient.name}
                >{$item.recipient.name || "Not set"}</span
              >
            </div>
          </div>
          <div class="info-item">
            <span class="title">Email</span>
            <div class="content">
              <span class:fade={!$item.recipient.email}
                >{$item.recipient.email || "Not set"}</span
              >
            </div>
          </div>
          ...
          <div class="info-item">
            <span class="title">Phone</span>
            <div class="content">
              <span class:fade={!$item.recipient.phone}
                >{$item.recipient.phone || "Not set"}</span
              >
            </div>
          </div>
          <div class="info-item">
            <span class="title">Address</span>
            <div class="content">
              <span class:fade={!$item.recipient.address}
                >{$item.recipient.address || "Not set"}</span
              >
            </div>
          </div>
        </div>
        ...
        <div class="col-span-2">
          <div>
            <div class="info-item">
              ...
              <div class="content">
                <span
                  >{dayjs($item.delivery.created_at).format(
                    "DD-MM-YYYY HH:mm A"
                  )}</span
                >
              </div>
          ...
          <div class="info-item">
            ...
            <div class="content">
              <span
                >{dayjs(
                  $item.delivery.completed_at || $item.delivery.updated_at
